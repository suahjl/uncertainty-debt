...
def do_everything_quadrant_thresholdsearch_fe(
    ...
    def quadrant_thresholdsearch_fe(
        ...
        for threshold0 in np.arange(
            ...
            for threshold1 in np.arange(
                ...
                # interactions
                df[
                    x_interactedwith_threshold_col + "_" + new_threshold_col_names[0]
                ] = (
                    df[x_interactedwith_threshold_col] * df[new_threshold_col_names[0]]
                )  # first
                df[
                    x_interactedwith_threshold_col + "_" + new_threshold_col_names[1]
                ] = (
    ...
    for mp_variable in tqdm(list_mp_variables):
        for uncertainty_variable in tqdm(list_uncertainty_variables):
            ...
            del df["date"]
            # Drop NA (also automatically clears countries with completely missing data columns)
            df = df.dropna(axis=0)
...
# III.A --- With EPU
# STIR
do_everything_quadrant_thresholdsearch_fe(
    ...
    cols_threshold=cols_threshold_priv_gov_ref,  # the ones with _ref
    threshold_variables=cols_threshold_priv_gov,  # the ones without _ref
    countries_drop=[
        "india",  # 2016 Q3
...
# # %%
# # TESTING
# df = pd.read_parquet(path_data + "data_macro_full_yoy.parquet")
# df = df[
#     ["country", "quarter", "stir", "maxminbrent", "uct_ref", "maxminwui", "maxminstir"]
#     + cols_endog_long
# ]
