...
if not manual_data:
    ...
    for col in tqdm(list(seriesids_all.columns)):
        try:
            ...
            seriesids = [
                i.replace(".0", "") for i in seriesids
            ]  # remove trailing decimals
            seriesids = [
                re.sub("[^0-9]+", "", i) for i in list(seriesids)
            ]  # keep only number
    ...
    for col, daily_or_not in zip(["brent"], [True]):
        df_global = pd.read_csv(
            path_data + "ceic_manual_download/global/" + col + ".txt"
        )
        df_global = process_ceic_df_global(
            df=df_global, col_label=col, daily=daily_or_not
        )
