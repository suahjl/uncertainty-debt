...
def thresholdsearch_fe(
    ...
    threshold_optimal = df_aicc.loc[
        df_aicc["aicc"] == df_aicc["aicc"].min(), "threshold"
    ].reset_index(drop=True)[0]
    print(df_aicc)
    ...
    # output
    return params_table, threshold_optimal, df_loglik, df_aicc
...
for mp_variable in tqdm(list_mp_variables):
    for uncertainty_variable in tqdm(list_uncertainty_variables):
        ...
        # IV --- Analysis
        # estimate model
        params_table_fe, threshold_optimal_fe, df_loglik_fe, df_aicc_fe = thresholdsearch_fe(
            data=df,
            y_col="gdp",
            threshold_input_col=threshold_variable + "_ref",
            new_threshold_col_name=threshold_variable + "_threshold",
            x_interactedwith_threshold_col=uncertainty_variable,
            other_x_cols=cols_all_endog + cols_all_exog + [mp_variable],
            threshold_range=[
                int(
                    df.groupby("country")[threshold_variable + "_ref"]
                    .quantile(0.2)
                    .min()
                ),
                int(
        ...
        file_name = (
            path_output
            + "reg_thresholdselection_ltir_reduced_fe_"
            + "modwith_"
            + uncertainty_variable
            + "_"
            + mp_variable
        ...
        df_opt_threshold = pd.DataFrame({threshold_variable: [threshold_optimal_fe]})
