        ...
        for uncertainty_variable in tqdm(list_uncertainty_variables):
            ...
            def find_quadrant_thresholds(
                ...
                if option == "reg_thresholdselection":
                    df_opt_threshold = pd.read_csv(
                        path_output
                        + "reg_quadrant_privdebt_thresholdselection_"  # same as privdebt + govdebt model with symmetric uncertainty shocks
                        + file_suffixes
                        + "fe_"
                        + "modwith_"
                        + uncertainty_variable
                    ...
                    # dummies for debt states and positive / negative uncertainty shock
                    # first, +ve uncertainty
                    df.loc[
                        (
                            (df[threshold_variables[0] + "_ref"] >= opt_threshold0)
                            & (df[uncertainty_variable] >= 0)
                        ),
                    ...
                    # positive uncertainty shocks
                    # quadrant 1 (0,0), +ve uncertainty
                    df.loc[
                        (
                            (
                                df[
            ...
            for var0, var0_nice in zip([0, 1], ["below threshold", "above threshold"]):
                for var1, var1_nice in zip(
                    ...
                    for uncertainty_direction in ["positive", "negative"]:
                        # estimate model
                        print(df)
                        ...
                        irf_on.to_parquet(
                            path_output
                            + "quadrant_privdebt_asym_panelthresholdlp_"
                            + file_suffixes
                            + "irf_on_"
                            + threshold_variables[0]
                            + str(var0)
            ...
            def plot_quadrant_irf(show_ci: bool):
                ...
                quadrant_width = [4, 2, 2, 4]  # 00, 10, 01, 11
                for shock in [uncertainty_variable, mp_variable]:
