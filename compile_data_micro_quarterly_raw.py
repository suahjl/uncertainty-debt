...
# I --- Preliminaries
path_data = "./data/"
path_spg = path_data + "spg_manual_download/quarterly/"
path_output = "./output/"
...
def process_spg_and_concat():
    ...
    for xlsx_filename in tqdm(xlsx_filenames):
        ...
        df = df.iloc[6:, :]
        # delete if FY0 is included by accident
        if "FY0" in df.columns:
            del df["FY0"]
        # convert into panel
        df = pd.wide_to_long(
            df=df,
            stubnames="CQ",  # calendar quarter
        ...
        df["quarter"] = (
            df["quarter"].str[1:]  # Extract YYYY
            + "Q"
            + df["quarter"].str[0]  # Add the first digit Q to after "Q"
        )
        df["quarter"] = pd.to_datetime(df["quarter"]).dt.to_period("Q")
        df["quarter"] = df["quarter"].astype("str")
        # merge / concat
        cols_identifiers = ["entity", "id", "country", "type", "quarter"]
        if count_df == 0:
            df_full = df.copy()
        elif count_df > 0:
...
df.to_parquet(path_data + "data_micro_quarterly_raw.parquet")  # , compression="brotli")
